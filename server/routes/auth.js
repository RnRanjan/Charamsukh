...
router.post('/register', [
  ...
  body('role').optional().isIn(['user', 'author']).withMessage('Invalid role')
], async (req, res) => {
  try {
...
router.get('/me', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId)
      .populate('bookmarkedStories', 'title author category')
      .populate('readingHistory.story', 'title author category');
  ...
  try {
    ...
    Object.keys(req.body).forEach(key => {
      if (allowedUpdates.includes(key)) {
        if (key === 'preferences') {
          updates[key] = { ...user.preferences, ...req.body[key] };
        } else {
